...
void NEATRunner::saveConstants(const std::filesystem::path &dir_path)
{
    ...
    constants["INIT_CONNECTIVITY_RATE"] = INIT_CONNECTIVITY_RATE;
...
{
    // Clear old frames from previous generations
    for (auto &frames : genFrames)
    {
        frames.clear();
    }
...
void NEATRunner::crossover()
{
    ...
    for (Species &species : speciesList)
    {
        ...
        if (numOffSpring > spotsLeft)
            numOffSpring = spotsLeft;
...
double NEATRunner::calcCompDistance(Genome &parent1, Genome &parent2)
{
    ...
    while (firstIt < parent1Size && secondIt < parent2Size)
    {
        ...
        else if (conn1.innvNum > conn2.innvNum)
        {
            ...
            if (conn2.innvNum > parent1InnovRange.first)
            { // if greater than it min innov its within range
                numDisjoint++;
            }
            else
            {
...
void NEATRunner::speciate()
{
    ...
    for (Genome &genome : genomes)
    {
        ...
        if (!foundMatch)
        { // create new species w/ this genome as its rep
            Species newSpecies;
